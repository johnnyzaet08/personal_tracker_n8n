...
Purpose: preserve and evolve the current finance foundation. Bank adapters remain blocked on representative anonymized fixtures. Gmail is a replaceable connector and is not part of the finance domain model.
...
- `automation.action_runs` lacks a direct `tenant_id` when no source event exists. Autonomous habit reminders must not reuse it until tenant isolation is made explicit.
